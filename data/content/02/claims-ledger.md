# Chapter 02 — The Crossing: Claims Ledger

The evidentiary backbone for chapter two (Hannibal's crossing of the Rhone and the
...
(see `route-candidates.md`) of attributing to a corpus query with locators left
...
- **Retrieved:** 2026-08-11
- **Contradicting sources:** **Livy 21.31 (TC-09b)** gives a different route — a standing route debate (conventions §2). See `route-candidates.md`.
...
Per the chapter directive: a claim with no retrievable supporting text is **kept
...
- **Status:** open debate, not a fact. Already carried as candidates in
  `route-candidates.md` (R1–R7). The chapter must present it as a debate, not
