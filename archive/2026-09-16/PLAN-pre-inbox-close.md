...
## Current focus
Astra review recorded 2026-09-10. Milestone 1, P1 Spine: the beat sheet for The Crossing. This is the single next actionable step, and it is writing, not pipeline. The claims ledger (`data/content/02/claims-ledger.md`, 19 anchored beats in narrative order) is the outline; P1 reorders those claims into narrative order, maps each beat to its TC ids, its map-journey module behaviour, and the geography it needs, and lands both standing disagreements as their own scenes (route TC-09a Polybius up-river vs TC-09b Livy's turn to the Druentia; blocking rock TC-15a Polybius carving vs TC-15b Livy fire and vinegar). A draft spec is ready at `specs/the-crossing-spine.md` (amended 2026-09-10, ten acceptance criteria); the building session freezes its criteria before starting. Do not open `site/poc/` or `data/geo/scripts/` this session: a commit touching those instead of `data/content/02/` is the relapse signal (see risks).
...
| Archaeological evidence: which marker? | Default: inferred with attribution (conventions v1.1 section 12) | first chapter rendering an archaeological feature |
...
| Should the conventions doc carry the locked Decision Log rule "world state follows the attestation of the time" (2026-08-20)? Neither v1.0 nor v1.1 has a clause for it. | The Decision Log row governs; conventions stay at v1.1 until Tony instructs a v1.2 amendment (section 4 or 7). | before P3 Scenes (Milestone 2) |
| How many chapters: twelve (the Chapter Tracker and `data/content/00` to `11`) or thirteen (the hub text)? | Work to twelve, the count the Chapter Tracker and repo carry; the hub sentence stays as written until Tony gives an instruction naming the hub page. | never blocks The Crossing; before any public page states a chapter count |
