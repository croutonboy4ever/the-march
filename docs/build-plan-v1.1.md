...
Version 1.1, 2026-09-10. Supersedes v1.0 (retained beside it as `build-plan-v1.0.md`). **This is the method.** Every chapter is built the same way,
in the same phases, against the same gates. The Crossing is the first instance.

Changes from v1.0 (Astra review recorded 2026-09-10, CHANGES.md rows 1 to 3):
- Section 2 gains a P2a Reading test phase between P2 and P3.
- The "Why this order" note gains one sentence on why P2a sits inside P2.
- Section 5 gains the P2a row and a P1 line naming Astra's beat-sheet review; the chip-density paragraph now names the P2a gate.
...
| **P2a Reading test** | The first three beats of the draft, marker-tagged, set on one static page at phone width in at least three treatments: per-clause chips (A / I / Im), one chip per passage with tap-through to the claims, plain-language labels (Ancient account / Reconstruction / Imagined scene). Same claims and anchors in every treatment; only the display changes | Preflight items 1 to 3 pass on the three beats; the page reads on a real phone without the narrative stopping; the treatment recommendation is written down and filed as a Decision Log row. P2 does not continue past three beats until this gate passes |
...
§9 says nothing ships without it. P2a sits inside P2 because the load-bearing assumption
in `BRIEF.md` (newcomer-readable and preflight-clean at once) is cheaper to test at three
beats than at a full chapter.
...
| P1 Spine | **Next** | Two standing disagreements must land as scenes: the route (TC-09a Polybius up-river vs TC-09b Livy's left turn to the Druentia) and the blocking rock (TC-15a Polybius carving vs TC-15b Livy's fire and vinegar). Astra reviews the beat sheet before the gate is called passed (CHANGES row 9) |
| P2 Draft | Blocked on P1 | Outward-facing prose. See the routing note below |
| P2a Reading test | Blocked on the first three beats of P2 | The three-beat reading test; gate in section 2 |
...
real chips on a real phone. This is not a decision to argue out; it is a test, and it is
the P2a gate, which sits inside P2.
